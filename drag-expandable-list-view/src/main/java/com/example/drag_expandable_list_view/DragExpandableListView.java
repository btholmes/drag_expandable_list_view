...
import com.example.drag_expandable_list_view.DragSortListView.DragSortItemView;
import com.example.drag_expandable_list_view.DragSortListView.DragSortItemViewCheckable;
import com.example.drag_expandable_list_view.DragSortListView.DragSortListView;
import com.example.drag_expandable_list_view.DragSortListView.SimpleFloatViewManager;
...
public class DragExpandableListView extends ListView {
    ...
    private ExpandableDragListener mDragListener;
    private ExpandableDropListener mDropListener;
    private ExpandableRemoveListener mRemoveListener;
    ...
    private DragExpandableDragScroller mDragScroller;
    ...
    private ExpandableDragScrollProfile mScrollProfile = new ExpandableDragScrollProfile() {
        @Override
    ...
    private ExpandableFloatViewManager mFloatViewManager = null;
    ...
    private ExpandableAdapterWrapper mAdapterWrapper;
...
//    private DragSortListView.DragSortTracker mDragSortTracker;
    ...
    public DragExpandableListView(Context ctx){
        super(ctx);
...
//
//            if (mTrackDragSort) {
//                mDragSortTracker = new DragSortListView.DragSortTracker();
//            }
            ...
            if (useDefault) {
                ...
                int removeMode = a.getInt(
                        ...
                        ExpandableDragSortController.FLING_REMOVE);
                ...
                int dragInitMode = a.getInt(
                        ...
                        ExpandableDragSortController.ON_DOWN);
                ...
                ExpandableDragSortController controller = new ExpandableDragSortController(
        ...
        mDragScroller = new DragExpandableDragScroller();

        float smoothness = 0.5f;
        if (removeAnimDuration > 0) {
            mRemoveAnimator = new DragExpandableListView.RemoveAnimator(smoothness, removeAnimDuration);
        }
        // mLiftAnimator = new LiftAnimator(smoothness, 100);
        if (dropAnimDuration > 0) {
            mDropAnimator = new DragExpandableListView.DropAnimator(smoothness, dropAnimDuration);
        }
...
//            }
    ...
    public void SharedConstructor(Context ctx, AttributeSet attrs){
    ...
    public interface ExpandableDragListener{
    ...
    public interface ExpandableRemoveListener{
     ...
     * {@link DragSortListView#setDragScrollProfile(DragSortListView.DragScrollProfile)} to
     * set custom profile.
     *
     * @author heycosmo
     *
     */
    public interface ExpandableDragScrollProfile {
        /**
     ...
     */
    public interface ExpandableFloatViewManager {
        /**
...
//        }
    ...
    public void setFloatViewManager(DragExpandableListView.ExpandableFloatViewManager manager) {
     ...
     */
    private class RemoveAnimator extends DragExpandableListView.SmoothAnimator {
    ...
    class DragExpandableDragScroller implements Runnable {
        ...
        public DragExpandableDragScroller() {
        }
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                DragExpandableListView.this.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
...
//                setSelectionFromTop(movePos, top-padTop);
            ...
            DragExpandableListView.this.layoutChildren();
            invalidate();
    ...
    private class ExpandableAdapterWrapper extends BaseAdapter {
        private ListAdapter mAdapter;

        public ExpandableAdapterWrapper(ListAdapter adapter) {
            super();
            mAdapter = adapter;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView != null) {
                ...
                child = mAdapter.getView(position, oldChild, DragExpandableListView.this);
                if (child != oldChild) {
                ...
                child = mAdapter.getView(position, null, DragExpandableListView.this);
     ...
     */
    private class LiftAnimator extends DragExpandableListView.SmoothAnimator {
     ...
     */
    private class DropAnimator extends DragExpandableListView.SmoothAnimator {
